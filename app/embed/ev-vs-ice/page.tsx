  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="block">
          <span className="text-xs font-medium text-gray-600">Annual km</span>
          <input
            type="number"
            value={annualKm}
            onChange={(e) => setAnnualKm(Number(e.target.value))}
            className="mt-1 w-full rounded-lg border border-gray-300 px-2 py-1.5 text-sm tabular-nums"
          />
        </label>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-xl bg-emerald-50 p-3">
          <div className="text-xs font-medium text-emerald-700">EV Cost</div>
          <div className="text-lg font-bold text-emerald-700 tabular-nums">
            {fmt(result.evAnnual, country)}
          </div>
          <div className="text-[10px] text-emerald-600">/year</div>
        </div>
      ...
      <p className="text-[10px] text-gray-400">
        Based on avg 15 kWh/100km (EV) and 7.5 L/100km (petrol) · <a href="https://battery.mom/calculators/ev-vs-ice" target="_blank" rel="noopener noreferrer" className="underline hover:text-emerald-600">Full calculator →</a>
      </p>
    </div>
  )
}
