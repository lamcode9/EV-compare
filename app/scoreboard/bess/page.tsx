    ...
    <nav className="mb-8 flex flex-wrap gap-2 text-sm" aria-label="Scoreboard views">
      {links.map((link) => (
        <Link
          ...
          className={
            link.active
              ? 'rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 font-semibold text-emerald-700'
              : 'rounded-full border border-gray-200 bg-white px-3 py-1.5 font-semibold text-gray-600 hover:border-emerald-200 hover:text-emerald-700'
          }
        >
          {link.label}
        </Link>
      ...
      <section className="container mx-auto max-w-7xl px-4 pb-16 pt-10 md:pt-12">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl font-bold text-gray-950 md:text-5xl">BESS Adoption Scoreboard</h1>
          ...
          <div className="rounded-xl border border-gray-200 bg-gray-950 p-5 text-white shadow-sm">
            <div className="text-xs font-semibold uppercase text-emerald-300">Global anchor</div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          <article className="rounded-xl border border-gray-200 bg-emerald-50/40 p-5">
            <div className="text-xs font-semibold uppercase text-emerald-700">2025 battery power build</div>
            <div className="mt-3 text-3xl font-bold text-gray-950">{formatGw(latestTotalGw)}</div>
            <p className="mt-2 text-sm text-gray-600">Utility-scale plus behind-the-meter additions.</p>
          </article>
          <article className="rounded-xl border border-gray-200 bg-white p-5">
            <div className="text-xs font-semibold uppercase text-gray-500">Utility-scale share</div>
            <div className="mt-3 text-3xl font-bold text-gray-950">
              {Math.round((latestPower.utilityScaleGw / latestTotalGw) * 100)}%
            </div>
            <p className="mt-2 text-sm text-gray-600">{formatGw(latestPower.utilityScaleGw)} of new 2025 power additions.</p>
          </article>
          <article className="rounded-xl border border-gray-200 bg-white p-5">
            <div className="text-xs font-semibold uppercase text-gray-500">YoY build growth</div>
            <div className="mt-3 text-3xl font-bold text-gray-950">+{annualGrowthPct.toFixed(0)}%</div>
            <p className="mt-2 text-sm text-gray-600">Growth in annual stationary battery power additions from 2024 to 2025.</p>
          </article>
        </div>

        <section className="mt-10">
          <div className="mb-5 max-w-3xl">
            <h2 className="text-xl font-bold text-gray-950">Sector adoption map</h2>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {sectors.map((sector) => (
              <article key={sector.name} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <h3 className="text-lg font-bold text-gray-950">{sector.name}</h3>
                  <span className="rounded-full border border-gray-200 bg-gray-50 px-2.5 py-1 text-[11px] font-semibold text-gray-600">
                    {sector.stage}
                  </span>
                </div>
                <div className="mt-4 rounded-lg bg-gray-50 p-4">
                  <div className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Primary metric to track</div>
                  <p className="mt-1 text-sm font-semibold text-gray-900">{sector.primaryMetric}</p>
                </div>
                <p className="mt-4 text-sm leading-6 text-gray-600">{sector.currentSignal}</p>
                <div className="mt-4 border-t border-gray-100 pt-4">
                  <div className="text-[11px] font-semibold uppercase tracking-wide text-amber-700">Data gap</div>
                  <p className="mt-1 text-sm leading-6 text-gray-600">{sector.missingData}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        <section className="mt-10 rounded-xl border border-gray-200 bg-gray-50 p-6">
          <h2 className="text-lg font-bold text-gray-950">Rules for this scoreboard</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            {sourceBoundaries.map((boundary) => (
              <div key={boundary} className="rounded-lg border border-gray-200 bg-white p-4 text-sm leading-6 text-gray-600">
                {boundary}
              </div>
            ))}
          </div>
