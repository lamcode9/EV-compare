...
export default function TenantROIPitch({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6 mb-8">
      ...
      {/* ── The pitch card ── */}
      <div ref={cardRef} className="bg-gradient-to-br from-emerald-50 to-blue-50 border-2 border-emerald-200 rounded-2xl p-6 mb-4">
        {/* Headline */}
        <div className="text-center mb-5">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-600 text-white rounded-full text-xs font-semibold mb-3">
            🏠 Green Home Investment
          </div>
          <h3 className="text-xl md:text-2xl font-bold text-gray-900 leading-tight">
            Your unit costs{' '}
            <span className="text-emerald-700">{fmtShort(effectiveCost, country)}</span> more
          </h3>
          <p className="text-lg text-gray-700 mt-1">
            but saves{' '}
            <span className="font-bold text-emerald-700">{fmt(effectiveSavings, country)}/month</span>{' '}
            <span className="text-gray-500 text-sm">({results.savingsAsPct.toFixed(0)}% of your electricity bill)</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white/70 rounded-xl p-3 text-center">
            <div className="text-2xl font-bold text-emerald-700">{results.effectivePayback.toFixed(1)}</div>
            <div className="text-xs text-gray-600 mt-0.5">Years to pay back</div>
          </div>
          <div className="bg-white/70 rounded-xl p-3 text-center">
            <div className="text-2xl font-bold text-blue-700">{fmtShort(results.roi20Year, country)}</div>
            <div className="text-xs text-gray-600 mt-0.5">20-year net gain</div>
          </div>
          <div className="bg-white/70 rounded-xl p-3 text-center">
            <div className="text-2xl font-bold text-amber-700">{blackoutHours}h</div>
            <div className="text-xs text-gray-600 mt-0.5">Blackout protection</div>
          </div>
          <div className="bg-white/70 rounded-xl p-3 text-center">
            <div className="text-2xl font-bold text-purple-700">{results.co2PerUnit.toFixed(1)}t</div>
            <div className="text-xs text-gray-600 mt-0.5">CO₂ saved/year</div>
          </div>
        </div>
