...
export default function DemandChargeHeatmap({ country, peakDemandKw: propPeak, targetReductionPct: propReduction, useCase: propUseCase }: Props) {
  ...
  const getColor = useCallback((kw: number, isShaved: boolean) => {
    ...
    if (ratio > 0.6) return 'bg-amber-300 text-gray-900'
    if (ratio > 0.4) return 'bg-yellow-200 text-gray-700'
    if (ratio > 0.2) return 'bg-lime-100 text-gray-600'
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6 mb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
        <div>
          <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Facility</label>
        ...
        <div>
          <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Peak demand</label>
        ...
        <div>
          <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-1">Target reduction</label>
          <div className="flex items-center gap-2">
        ...
        <div className="flex items-end pb-1">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-gray-700">Show battery shaving</span>
          </label>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
          <div className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wide">Monthly saving</div>
          <div className="text-xl font-bold text-emerald-900">{fmtDemandCharge(monthlySaving)}</div>
          <div className="text-[10px] text-emerald-500">Demand charge only</div>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-[10px]">
          <thead>
            <tr>
              <th className="w-10 text-left text-gray-500 py-1">Day</th>
              {Array.from({ length: 24 }, (_, h) => (
                <th key={h} className="py-1 px-0 text-center text-gray-400 font-normal" style={{ minWidth: 28 }}>
          ...
          <tbody>
            {DAYS.map((day, d) => (
              <tr key={day}>
                <td className="text-gray-600 font-medium pr-1 py-0.5">{day}</td>
                {data[d].map((kw, h) => {
                  ...
                  return (
                    <td key={h}
                      className={`relative py-0.5 px-0 text-center transition-all cursor-default
                        ...
                        ${isHover ? 'ring-2 ring-gray-800 ring-inset z-10' : ''}`}
                      ...
                      {/* Tooltip */}
                      {isHover && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 z-50 bg-gray-900 text-white rounded-lg px-3 py-2 text-[11px] whitespace-nowrap shadow-xl pointer-events-none">
      ...
      <div className="flex flex-wrap items-center gap-4 mt-3 text-[10px] text-gray-500">
        <span className="font-medium text-gray-600">Demand level:</span>
        <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-green-50 border" /> &lt;20%</span>
